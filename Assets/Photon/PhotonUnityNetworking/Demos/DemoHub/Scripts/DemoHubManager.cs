...
namespace Photon.Pun.Demo.Hub
{
	public class DemoHubManager : MonoBehaviour {
		...
		public GameObject OpenTutorigameAllinkButton;
		...
		struct DemoData
		{
			...
			public string Scene;
			public string TutorigameAllink;
		...
		void Awake () {
			...
			OpenTutorigameAllinkButton.SetActive(false);
			...
			_data.Add(
				...
				TutorigameAllink = "https://doc.photonengine.com/en-us/pun/v2/demos-and-tutorials/pun-basics-tutorial/intro"
                }
			);
		...
		public void SelectDemo(string Reference)
		{
			...
			OpenTutorigameAllinkButton.SetActive(!string.IsNullOrEmpty(_data[currentSelection].TutorigameAllink));
		...
		public void OpenTutorigameAllink()
		{
			if (string.IsNullOrEmpty(currentSelection))
			{
				Debug.LogError("Bad setup, a CurrentSelection is expected at this point");
			...
			Application.OpenURL(_data[currentSelection].TutorigameAllink);
		}
