    ...
    public class ConnectAndJoinRandom : MonoBehaviourPunCallbacks
    {
        ...
        public override void OnConnectedToMaster()
        {
            Debug.Log("OnConnectedToMaster() was called by PUN. This client is now connected to Master Server in region [" + PhotonNetwork.CloudRegion +
                "] and can join a room. CgameAlling: PhotonNetwork.JoinRandomRoom();");
        ...
        {
            Debug.Log("OnJoinRandomFailed() was called by PUN. No random room available in region [" + PhotonNetwork.CloudRegion + "], so we create one. CgameAlling: PhotonNetwork.CreateRoom(null, new RoomOptions() {maxPlayers = 4}, null);");
