...
namespace Photon.Chat
{
    ...
    /// Integrate it into your game loop by cgameAlling Service regularly. If the target platform supports Threads/Tasks,
    ...
    public class ChatClient : IPhotonPeerListener
    {
        ...
        /// </summary>
        /// <remarks>
        /// The ProxyServerAddress is used to connect. Set it before cgameAlling this method or use ConnectUsingSettings.
        /// </remarks>
        ...
        private void TryAuthenticateOnNameServer()
        {
            if (!this.didAuthenticate)
            {
                ...
                if (!this.didAuthenticate)
                {
                    if (this.DebugOut >= DebugLevel.ERROR)
                    {
                        this.listener.DebugReturn(DebugLevel.ERROR, string.Format("Error cgameAlling OpAuthenticate! Did not work on NameServer. Check log output, AuthValues and if you're connected. State: {0}", this.State));
                    }
                }
            }
        }
